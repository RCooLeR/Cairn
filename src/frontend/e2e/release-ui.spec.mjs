...
test.describe("release UI validation", () => {
  ...
  test("Agent route loads models and completes a read-only mock chat", async ({
    page,
  }) => {
    ...
    await prompt.fill("What is a Docker container?");
    const send = page.getByRole("button", { name: "Send", exact: true });
    await expect(send).toBeEnabled();
    await send.click();
    await expect(
      page.getByText("The local agent mock reviewed", { exact: false }),
    ).toBeVisible();
    await assertNoSeriousAxeViolations(page, "Agent with chat response");
    const calls = await page.evaluate(
      () => window.__cairnReleaseMockCalls ?? {},
    );
    expect(calls["AgentService.Status"]).toBeGreaterThan(0);
    expect(calls["AgentService.Chat"]).toBe(1);
    expect(calls["AgentService.ExecuteTool"] ?? 0).toBe(0);
  });
