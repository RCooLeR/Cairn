...
describe("inventory store slice ownership", () => {
  ...
  it("invalidates a provider scope immediately while deferring hidden-window reads", async () => {
    await useInventoryStore.getState().refresh();
    vi.mocked(getInventorySnapshot).mockClear();

    await useInventoryStore.getState().refreshScope({ deferRefresh: true });

    expect(getInventorySnapshot).not.toHaveBeenCalled();
  ...
  it("does not let an older freshness waiter restart a deferred provider refresh", async () => {
    const oldRead = deferred<InventorySnapshot>();
    vi.mocked(getInventorySnapshot)
      .mockReturnValueOnce(oldRead.promise)
      .mockResolvedValueOnce(
        snapshot({ containers: [container("new-provider")] }),
      );
    const oldRefresh = useInventoryStore.getState().refreshFresh();

    await useInventoryStore.getState().refreshScope({ deferRefresh: true });
    oldRead.resolve(snapshot({ containers: [container("old-provider")] }));
    await oldRefresh;
