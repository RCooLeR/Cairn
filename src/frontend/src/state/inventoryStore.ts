...
export type InventoryState = {
  ...
  refreshScope: (options?: { deferRefresh?: boolean }) => Promise<void>;
...
export const useInventoryStore = create<InventoryState>((set, get) => ({
  ...
  networkDetails: {},
  refresh: () => {
    fullRefreshDeferred = false;
    return startOrJoinFullRefresh(set);
  },
  refreshFresh: () => requestFreshInventory(set, false),
  refreshScope: (options) =>
    requestFreshInventory(set, true, options?.deferRefresh),
  refreshContainers: () =>
    refreshSlice(
      "containers",
...
async function requestFreshInventory(
  ...
  freshnessEpoch += 1;
  fullRefreshDeferred = deferRefresh;
  if (resetScope) {
    connectionEpoch += 1;
  ...
  // Clear obsolete resources immediately even when the window is hidden.
  // Its next foreground refresh loads this epoch without waking Docker now.
  if (deferRefresh) {
    return;
  }
  ...
  while (!fullRefreshDeferred && completedFreshnessEpoch < freshnessEpoch) {
...
async function refreshSlice<Slice extends RefreshableSlice>(
  ...
  try {
    const value = await boundedRead(load, `Refresh ${slice}`);
    const completedAt = Date.now();
    set((state) => {
...
async function loadVolumeDetail(
  ...
  try {
    ...
    try {
      detail = await boundedRead(
        () => DockerService.GetVolume(name),
        "Volume details",
      );
    } catch (error) {
      const state = get();
