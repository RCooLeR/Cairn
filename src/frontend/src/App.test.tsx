...
describe("App inventory shell", () => {
  ...
  it("ignores an older dashboard read failure after a newer event refresh succeeds", async () => {
    inventoryMock.getInventorySnapshot.mockResolvedValue(seededSnapshot());
    const oldRead = deferred<DashboardMetrics>();
    metricsServiceMock.GetDashboardMetrics.mockReturnValueOnce(oldRead.promise);
    render(<App />);
    await screen.findByText("Docker Engine - Running");
    await waitFor(() =>
      expect(metricsServiceMock.GetDashboardMetrics).toHaveBeenCalledTimes(1),
    );

    emitRuntimeEvent("objects:changed", { kind: "network", ids: [] });
    await waitFor(() =>
      expect(metricsServiceMock.GetDashboardMetrics).toHaveBeenCalledTimes(2),
    );
    expect(await screen.findByText("18% / 2.0 GiB")).toBeInTheDocument();
    await act(async () => {
      oldRead.reject(new Error("Obsolete dashboard failure"));
    });
    expect(
      screen.queryByText("Obsolete dashboard failure"),
    ).not.toBeInTheDocument();
  });
  ...
  it("disposes hidden terminal surfaces and listeners when provider scope changes", async () => {
    ...
    // The tab appears before the lazy terminal renderer has loaded.
    await waitFor(() => expect(xtermMock.nextInstanceID).toBeGreaterThan(0));
    ...
    // Shared app-lifetime capture stays attached; the old terminal surface
    // unsubscribes from it and must not render obsolete session output below.
    expect(
      runtimeMock.on.mock.calls.filter(
  ...
  it("opens a full container detail screen from the containers table", async () => {
    ...
    act(() => {
      useInventoryStore
        .getState()
        .setContainers([
          { ...container, health: HealthStatus.HealthStatusStarting },
        ]);
    });
    expect(screen.getByText("Health").parentElement).toHaveTextContent(
      "starting",
    );
    act(() => {
      useInventoryStore
        .getState()
        .setContainers([
          { ...container, health: HealthStatus.HealthStatusHealthy },
        ]);
    });
    expect(screen.getByText("Health").parentElement).toHaveTextContent(
      "healthy",
  ...
  it.each(["obsolete", "empty"])(
    "does not announce success for an %s container terminal open",
    async (outcome) => {
      const snapshot = seededSnapshot();
      const pending = deferred<TerminalSessionInfo | null>();
      inventoryMock.getInventorySnapshot.mockResolvedValue(snapshot);
      terminalServiceMock.OpenContainerTerminal.mockReturnValue(
        pending.promise,
      );
      render(<App />);
      await screen.findByText("Docker Engine - Running");
      ...
      fireEvent.click(
        screen.getByRole("button", {
          name: `Open ${snapshot.containers[0].name} container details`,
        }),
      );
      fireEvent.click(screen.getByRole("button", { name: "Open terminal" }));
      expect(terminalServiceMock.OpenContainerTerminal).toHaveBeenCalledOnce();
      if (outcome === "obsolete") {
        emitRuntimeEvent("provider:changed", {});
      }
      await act(async () => {
        pending.resolve(
          outcome === "empty"
      ...
      expect(
        screen.queryByText("Container terminal opened"),
      ).not.toBeInTheDocument();
      if (outcome === "obsolete") {
        ...
        expect(
          screen.getByText("Container terminal returned no session"),
        ).toBeInTheDocument();
      }
    },
  );
  ...
  it.each(["container", "project", "all"])(
    "refreshes project health after %s reconciliation completes",
    async (kind) => {
      const starting = seededProjectDetail();
      starting.services = starting.services?.map((service, index) => ({
        ...service,
        health:
          index === 0
      ...
      inventoryMock.getInventorySnapshot.mockResolvedValue(seededSnapshot());
      projectServiceMock.RefreshProjects.mockResolvedValue([seededProject()]);
      projectServiceMock.GetProject.mockResolvedValue(starting);
      render(<App />);
      await screen.findByText("Docker Engine - Running");
      fireEvent.click(
        within(
      ...
      fireEvent.click(await screen.findByRole("button", { name: "app-db" }));
      await screen.findByText("Health: checking");
      expect(screen.getByText("Health: unhealthy")).toBeInTheDocument();
      const reconciliation = deferred<ProjectSummary[]>();
      projectServiceMock.RefreshProjects.mockReturnValueOnce(
      ...
      emitRuntimeEvent(
        "objects:changed",
        kind === "all" ? undefined : { kind, ids: [] },
      );
      await act(async () => {
        await vi.advanceTimersByTimeAsync(500);
      });
      expect(projectServiceMock.GetProject).toHaveBeenCalledTimes(1);
      expect(screen.getByText("Health: checking")).toBeInTheDocument();
      await act(async () => {
        reconciliation.resolve([seededProject()]);
        await vi.advanceTimersByTimeAsync(0);
      });
      expect(projectServiceMock.GetProject).toHaveBeenCalledTimes(2);
      expect(screen.getByText("Health: healthy")).toBeInTheDocument();
      expect(screen.queryByText("Health: checking")).not.toBeInTheDocument();
    },
  );

  it("refreshes project health when a newer list-only refresh supersedes its reconciliation", async () => {
    const starting = seededProjectDetail();
    starting.services = starting.services?.map((service) => ({
      ...service,
      health: HealthStatus.HealthStatusStarting,
    }));
    inventoryMock.getInventorySnapshot.mockResolvedValue(seededSnapshot());
    projectServiceMock.RefreshProjects.mockResolvedValue([seededProject()]);
    projectServiceMock.GetProject.mockResolvedValue(starting);
    ...
    fireEvent.click(await screen.findByRole("button", { name: "app-db" }));
    await screen.findAllByText("Health: checking");
    const healthRefresh = deferred<ProjectSummary[]>();
    const listRefresh = deferred<ProjectSummary[]>();
    projectServiceMock.RefreshProjects.mockReturnValueOnce(
      healthRefresh.promise,
    ).mockReturnValueOnce(listRefresh.promise);
    projectServiceMock.GetProject.mockResolvedValue(seededProjectDetail());
    const globalRefresh = screen.getByLabelText("Refresh", {
      selector: "button",
    });
    vi.useFakeTimers();
    emitRuntimeEvent("objects:changed", { kind: "container", ids: [] });
    // Model a queued header click before React commits the loading/disabled
    // state from the health-event reconciliation in this same event turn.
    act(() => {
      vi.advanceTimersByTime(500);
      fireEvent.click(globalRefresh);
    });
    expect(projectServiceMock.RefreshProjects).toHaveBeenCalledTimes(3);
    const latestProject = { ...seededProject(), name: "latest-app" };
    await act(async () => {
      listRefresh.resolve([latestProject]);
      await vi.advanceTimersByTimeAsync(0);
    });
    expect(projectServiceMock.GetProject).toHaveBeenCalledTimes(1);
    await act(async () => {
      healthRefresh.resolve([seededProject()]);
      await vi.advanceTimersByTimeAsync(0);
    });
    expect(projectServiceMock.GetProject).toHaveBeenCalledTimes(2);
    expect(screen.getByText("Health: healthy")).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "Back" }));
    expect(
      screen.getByRole("button", { name: "latest-app" }),
    ).toBeInTheDocument();
  });

  it.each(["close", "switch", "provider"])(
    "does not reopen old project detail after reconciliation and %s",
    async (change) => {
      const projectA = seededProject();
      const projectB = alternateProject();
      inventoryMock.getInventorySnapshot.mockResolvedValue(seededSnapshot());
      ...
      projectServiceMock.GetProject.mockImplementation((id: string) =>
        Promise.resolve(
          projectDetailFor(id === projectB.id ? projectB : projectA),
        ),
      );
      render(<App />);
      await screen.findByText("Docker Engine - Running");
      ...
      if (change === "provider") {
        ...
        if (change === "switch") {
          ...
          fireEvent.click(
            screen.getAllByRole("button", { name: /api-worker/ })[0],
          );
          fireEvent.click(
      ...
      await act(async () => {
        reconciliation.resolve([projectA, projectB]);
        await vi.advanceTimersByTimeAsync(0);
      });
      expect(
        projectServiceMock.GetProject.mock.calls.filter(
          ([id]) => id === projectA.id,
        ),
      ).toHaveLength(1);
      if (change === "switch") {
  ...
  it("defers Docker inventory, project discovery, metrics, and log preview while initially hidden", async () => {
    ...
    expect(inventoryMock.getInventorySnapshot).not.toHaveBeenCalled();
    expect(projectServiceMock.RefreshProjects).not.toHaveBeenCalled();
    expect(metricsServiceMock.StartStatsStream).not.toHaveBeenCalled();
    expect(metricsServiceMock.GetDashboardMetrics).not.toHaveBeenCalled();
    expect(logsServiceMock.StartLogStream).not.toHaveBeenCalled();
    act(() => {
      emitRuntimeEvent("docker:connected", {});
      emitRuntimeEvent("objects:changed", { kind: "container" });
      emitRuntimeEvent("objects:changed", { kind: "image" });
    });
    expect(inventoryMock.getInventorySnapshot).not.toHaveBeenCalled();
    act(() => {
    ...
    await waitFor(() =>
      expect(metricsServiceMock.StartStatsStream).toHaveBeenCalledTimes(1),
    );
    expect(inventoryMock.getInventorySnapshot).toHaveBeenCalledTimes(1);
    expect(projectServiceMock.RefreshProjects).toHaveBeenCalledTimes(1);
    expect(dockerServiceMock.ListContainers).not.toHaveBeenCalled();
    expect(dockerServiceMock.ListImages).not.toHaveBeenCalled();
    unmount();
  ...
  it("stops hidden metrics, ignores late samples, and resumes a new stream on show", async () => {
    const visibility = vi.spyOn(document, "visibilityState", "get");
    visibility.mockReturnValue("visible");
    inventoryMock.getInventorySnapshot.mockResolvedValue(seededSnapshot());
    metricsServiceMock.StartStatsStream.mockResolvedValueOnce(
      "visible-stream-1",
    ).mockResolvedValueOnce("visible-stream-2");
    const { unmount } = render(<App />);
    await screen.findByText("Docker Engine - Running");
    await waitFor(() =>
      expect(metricsServiceMock.StartStatsStream).toHaveBeenCalledTimes(1),
    ...
    await waitFor(() =>
      expect(logsServiceMock.StartLogStream).toHaveBeenCalledTimes(1),
    );
    const initialLogStreamID =
      await logsServiceMock.StartLogStream.mock.results[0].value;
    act(() =>
      emitRuntimeEvent("stats:sample", {
        streamID: "visible-stream-1",
        samples: [statsSample({ cpuPercent: 27 })],
      }),
    );
    expect(useInventoryStore.getState().containers[0].cpuPercent).toBe(27);

    act(() => {
      visibility.mockReturnValue("hidden");
      document.dispatchEvent(new Event("visibilitychange"));
    });
    expect(metricsServiceMock.StopStream).toHaveBeenCalledExactlyOnceWith(
      "visible-stream-1",
    );
    expect(logsServiceMock.StopStream).toHaveBeenCalledWith(initialLogStreamID);
    act(() =>
      emitRuntimeEvent("stats:sample", {
        streamID: "visible-stream-1",
        samples: [statsSample({ cpuPercent: 99 })],
      }),
    );
    expect(useInventoryStore.getState().containers[0].cpuPercent).toBe(27);
    ...
    await waitFor(() =>
      expect(metricsServiceMock.StartStatsStream).toHaveBeenCalledTimes(2),
    );
    expect(logsServiceMock.StartLogStream).toHaveBeenCalledTimes(2);
    act(() =>
      emitRuntimeEvent("stats:sample", {
        streamID: "visible-stream-2",
        samples: [statsSample({ cpuPercent: 31 })],
      }),
    );
    expect(useInventoryStore.getState().containers[0].cpuPercent).toBe(31);
    unmount();
  });

  it("clears a hidden provider scope immediately and performs one full refresh on show", async () => {
    const visibility = vi.spyOn(document, "visibilityState", "get");
    visibility.mockReturnValue("visible");
    inventoryMock.getInventorySnapshot.mockResolvedValue(seededSnapshot());
    const { unmount } = render(<App />);
    await screen.findByText("Docker Engine - Running");
    inventoryMock.getInventorySnapshot.mockClear();
    projectServiceMock.RefreshProjects.mockClear();
    act(() => {
      visibility.mockReturnValue("hidden");
      document.dispatchEvent(new Event("visibilitychange"));
      emitRuntimeEvent("provider:changed", {});
  ...
  it("stops a metrics stream that finishes opening after the window hides", async () => {
    const visibility = vi.spyOn(document, "visibilityState", "get");
    visibility.mockReturnValue("visible");
    inventoryMock.getInventorySnapshot.mockResolvedValue(seededSnapshot());
    const opening = deferred<string>();
    metricsServiceMock.StartStatsStream.mockReturnValueOnce(opening.promise);
    const { unmount } = render(<App />);
    await screen.findByText("Docker Engine - Running");
    await waitFor(() =>
      expect(metricsServiceMock.StartStatsStream).toHaveBeenCalled(),
    ...
    await act(async () => opening.resolve("hidden-late-stream"));
    expect(metricsServiceMock.StopStream).toHaveBeenCalledExactlyOnceWith(
      "hidden-late-stream",
    );
    unmount();
  ...
  it("refreshes every affected resource when object event kinds arrive together", async () => {
    inventoryMock.getInventorySnapshot.mockResolvedValue(seededSnapshot());
    render(<App />);
    await screen.findByText("Docker Engine - Running");
    vi.useFakeTimers();
  ...
  it("trails in-flight inventory with a fresh read after an object change", async () => {
    inventoryMock.getInventorySnapshot.mockResolvedValue(seededSnapshot());
    render(<App />);
    await screen.findByText("Docker Engine - Running");
    const oldRead = deferred<InventorySnapshot>();
    const latest = seededSnapshot();
    latest.containers = [];
    inventoryMock.getInventorySnapshot
    ...
    await act(async () => {
      await vi.advanceTimersByTimeAsync(500);
      oldRead.resolve(seededSnapshot());
