...
export function SettingsPage({
  ...
  const refreshDockerShimStatus = useCallback(
    async (identity?: string) => {
      ...
      return runDockerShimRequest(async () => {
        const status = await boundedRead(
          () => SettingsService.GetWindowsDockerCLIShimStatus(),
          "Docker CLI shim status",
        );
        if (!status) {
  ...
  const refreshRuntimeDiagnostics = useCallback(async () => {
    await runRuntimeDiagnosticsRequest(async () => {
      const diagnostics = await boundedRead(
        () => DiagnosticsService.GetRuntimeDiagnostics(),
        "Runtime diagnostics",
      );
      if (!diagnostics) {
