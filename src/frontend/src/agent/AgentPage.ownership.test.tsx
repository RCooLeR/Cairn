...
describe("AgentPage conversation ownership", () => {
  ...
  it.each(["throws", "rejects"])(
    "releases a stopped request even when cancellation %s",
    async (failure) => {
      const chat = deferred<AgentChatResponse>();
      const cancel = vi.fn(() => {
        if (failure === "throws") {
      ...
      agentServiceMock.Chat.mockReturnValue(
        Object.assign(chat.promise, { cancel }),
      );
      render(<AgentPage projects={agentProjects()} />);
      fireEvent.change(
        screen.getByRole("textbox", { name: "Message to Cairn Agent" }),
        {
          target: { value: "Explain Docker" },
        },
      );
      const send = screen.getByRole("button", { name: "Send" });
      await waitFor(() => expect(send).toBeEnabled());
      ...
      await act(async () => {
        chat.resolve(new AgentChatResponse({ message: "Late result" }));
        await chat.promise;
      });
  ...
  it("unlocks the conversation when a stopped post-tool follow-up never settles", async () => {
    const followup = deferred<AgentChatResponse>();
    const cancel = vi.fn();
    agentServiceMock.Chat.mockResolvedValueOnce(
      toolRequestResponse(),
    ).mockReturnValueOnce(Object.assign(followup.promise, { cancel }));
    render(<AgentPage projects={agentProjects()} />);
    fireEvent.change(
      screen.getByRole("textbox", { name: "Message to Cairn Agent" }),
      {
        target: { value: "Inspect containers" },
      },
    );
    const send = screen.getByRole("button", { name: "Send" });
    await waitFor(() => expect(send).toBeEnabled());
    ...
    expect(screen.getByLabelText("Project")).toBeEnabled();
    await act(async () => {
      followup.resolve(toolRequestResponse());
