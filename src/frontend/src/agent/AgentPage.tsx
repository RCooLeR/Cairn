...
export function resetAgentSessionForTest() {
  cancelAgentRequest(
    useAgentSessionStore.getState().activeRequest,
    "Agent test session reset",
  );
...
function cancelAgentRequest(request: AgentChatRequest | null, reason: string) {
  try {
    void Promise.resolve(request?.cancel?.(reason)).catch(() => undefined);
  } catch {
    // Request ownership is invalidated independently of bridge cancellation.
  }
}
...
export function AgentPage({ projects }: AgentPageProps) {
  ...
  const refreshAgent = useCallback(async (showSpinner = true) => {
    ...
    try {
      const [nextStatus, nextTools] = await Promise.all([
        boundedRead(() => AgentService.Status(), "Local agent status"),
        boundedRead(() => AgentService.ToolCatalog(), "Local agent tools"),
      ]);
      if (!isCurrent()) {
        return;
      }
  ...
  const loadProjectAnalysis = useCallback(
    async (targetProjectID = projectID) => {
      ...
      setEditError(null);
      try {
        const nextAnalysis = await boundedRead(
          () => AgentService.AnalyzeProject(targetProjectID),
          "Project analysis",
        );
  ...
  const stopPrompt = () => {
    ...
    if (session.inFlightToolCall) {
      // An active chat after approval is the follow-up, so the approved tool
      // itself has already finished. Do not keep its UI lock until a bridge
      // cancellation eventually settles (or never returns).
      session.finishToolCall(session.inFlightToolCall);
    }
    cancelAgentRequest(session.activeRequest, "Stopped by user");
