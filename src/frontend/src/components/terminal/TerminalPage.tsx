...
import { encodeTerminalInput } from "./terminalEncoding";
import {
...
export function TerminalPage({
  ...
  useEffect(() => {
    const off = Events.On("terminal:closed", (event) => {
      ...
      closedSessionIDsRef.current.add(payload.sessionID);
      if (closedSessionIDsRef.current.size > 1024) {
        const oldest = closedSessionIDsRef.current.values().next().value;
        if (oldest) {
          closedSessionIDsRef.current.delete(oldest);
        }
      }
    ...
    boundedRead(
      ...
      .catch((shellError: unknown) => {
        if (active) {
          ...
          setContainerShell("");
          setError(
            errorMessage(shellError, "Unable to detect container shells"),
          );
        }
      });
    return () => {
  ...
  const addSession = useCallback((session: TerminalSessionInfo | null) => {
    if (
      !session ||
      closedSessionIDsRef.current.has(session.id) ||
      isTerminalSessionClosed(session.id)
    ) {
      return null;
    }
    if (!initialSessionsLoadedRef.current) {
  ...
  const openProject = useCallback(async () => {
    if (!selectedProjectID || openingRef.current) {
      return;
    }
    openingRef.current = true;
    setBusy(true);
  ...
  const openContainer = useCallback(async () => {
    if (!selectedTerminalContainerID || openingRef.current) {
      return;
    }
    openingRef.current = true;
    setBusy(true);
      ...
      <section className="flex min-h-[520px] min-w-0 flex-col overflow-hidden rounded-card border border-border bg-bg-panel xl:min-h-0">
        <div className="space-y-2 border-b border-border px-3 py-3">
          ...
          <div className="grid gap-2 lg:grid-cols-2 xl:grid-cols-[minmax(0,1fr)_156px_110px_minmax(120px,0.45fr)_auto]">
            ...
            >
              <option value="">Auto-detect shell</option>
              {shellOptions.filter(Boolean).map((shell) => (
        ...
        <div className="relative min-h-0 flex-1 bg-terminal-bg">
          <Suspense
            fallback={
              <div className="p-4 text-text-muted" role="status">
                Loading terminal…
              </div>
            }
          >
...
function mergeInitialTerminalSessions(
  ...
  const sessions = listed.filter(
    (session) =>
      !locallyRemoved.has(session.id) && !isTerminalSessionClosed(session.id),
  );
