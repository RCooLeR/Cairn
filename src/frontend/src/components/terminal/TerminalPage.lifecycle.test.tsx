...
describe("TerminalPage operation and session lifecycle", () => {
  ...
  it("replays shell output emitted before the open request resolves", async () => {
    ...
    act(() => {
      emit("terminal:data", {
        dataBase64: btoa("root@ollama:/# "),
        sessionID: "early-output",
      });
    });
    await act(async () => {
  ...
  it("bounds replay memory and preserves output while the terminal surface is absent", async () => {
    renderTerminalPage();
    await waitFor(() =>
    ...
    act(() => {
      emit("terminal:data", {
        sessionID: "buffered",
        dataBase64: btoa("x".repeat(140 * 1024)),
      });
      emit("terminal:data", {
        sessionID: "buffered",
    ...
    const unsubscribe = subscribeTerminalOutput("buffered", (data) =>
      replay.push(data),
    );
    expect(replay.reduce((size, chunk) => size + chunk.length, 0)).toBe(
      128 * 1024,
    );
    expect(new TextDecoder().decode(replay[replay.length - 1])).toBe(
      "prompt> ",
    );
    unsubscribe();
    act(() =>
      emit("terminal:data", {
        sessionID: "buffered",
        dataBase64: btoa("during navigation"),
      }),
    );
    const nextReplay: Uint8Array[] = [];
    const unsubscribeNext = subscribeTerminalOutput("buffered", (data) =>
      nextReplay.push(data),
    );
    expect(new TextDecoder().decode(nextReplay[nextReplay.length - 1])).toBe(
      "during navigation",
    );
    unsubscribeNext();
    act(() => {
      for (let i = 0; i < 2000; i += 1) {
        emit("terminal:data", { sessionID: "tiny-chunks", dataBase64: "" });
        emit("terminal:data", {
          sessionID: "tiny-chunks",
          dataBase64: btoa("x"),
        });
      }
    });
    const tinyReplay: Uint8Array[] = [];
    const unsubscribeTiny = subscribeTerminalOutput("tiny-chunks", (data) =>
      tinyReplay.push(data),
    );
    expect(tinyReplay).toHaveLength(512);
    expect(tinyReplay.every((chunk) => chunk.length === 1)).toBe(true);
    unsubscribeTiny();
    act(() =>
      emit("terminal:closed", { sessionID: "tiny-chunks", exitCode: 0 }),
    );
    expect(isTerminalSessionClosed("tiny-chunks")).toBe(true);
  });

  it("does not let empty output evict buffered prompts", async () => {
    renderTerminalPage();
    await waitFor(() =>
      expect(runtimeMock.listeners.get("terminal:data")?.size).toBe(1),
    );
    act(() => {
      ...
      for (let i = 0; i < 64; i += 1) {
        emit("terminal:data", { sessionID: `empty-${i}`, dataBase64: "" });
      }
    });
    const replay: Uint8Array[] = [];
    const unsubscribe = subscribeTerminalOutput("prompt", (data) =>
      replay.push(data),
    );
    expect(replay).toHaveLength(1);
    expect(new TextDecoder().decode(replay[0])).toBe("ready> ");
    unsubscribe();
  });

  it("reports a stalled session read instead of waiting forever", async () => {
    vi.useFakeTimers();
    terminalServiceMock.ListTerminalSessions.mockReturnValueOnce(
      new Promise(() => {}),
    );
    const page = renderTerminalPage();
    await act(async () => {
      await vi.advanceTimersByTimeAsync(30_000);
    });
    expect(
      screen.getByText("Loading terminal sessions timed out. Please retry."),
    ).toBeInTheDocument();
    page.unmount();
    vi.useRealTimers();
  });

  it("does not resurrect a session that exited before its open response", async () => {
    const pendingOpen = deferred<TerminalSessionInfo | null>();
    terminalServiceMock.OpenHostTerminal.mockReturnValueOnce(
      pendingOpen.promise,
    );
    ...
    fireEvent.click(await screen.findByRole("button", { name: "Host" }));
    act(() =>
      emit("terminal:closed", { exitCode: 0, sessionID: "already-exited" }),
    );
    await act(async () => {
      pendingOpen.resolve(terminalSession({ id: "already-exited" }));
      await pendingOpen.promise;
    });
    expect(screen.queryByRole("tab", { name: "Host" })).not.toBeInTheDocument();
  });

  it("clears the previous container's shell and serializes repeated Enter opens", async () => {
    const pendingShells = deferred<string[]>();
    const pendingOpen = deferred<TerminalSessionInfo | null>();
    terminalServiceMock.DetectContainerShells.mockResolvedValueOnce([
      "/bin/bash",
    ]).mockReturnValueOnce(pendingShells.promise);
    terminalServiceMock.OpenContainerTerminal.mockReturnValueOnce(
      pendingOpen.promise,
    );
    renderTerminalPage({
      containers: [terminalContainer("ubuntu"), terminalContainer("alpine")],
    });
    const containerPicker = screen.getByRole("combobox", {
      name: "Container terminal",
    });
    fireEvent.change(containerPicker, { target: { value: "ubuntu" } });
    await waitFor(() =>
      expect(
        screen.getByRole("combobox", { name: "Container shell path" }),
      ).toHaveValue("/bin/bash"),
    );
    fireEvent.change(containerPicker, { target: { value: "alpine" } });
    expect(
      screen.getByRole("combobox", { name: "Container shell path" }),
    ).toHaveValue("");
    fireEvent.keyDown(containerPicker, { key: "Enter" });
    fireEvent.keyDown(containerPicker, { key: "Enter" });
    expect(terminalServiceMock.OpenContainerTerminal).toHaveBeenCalledTimes(1);
    expect(terminalServiceMock.OpenContainerTerminal).toHaveBeenCalledWith(
      "alpine",
      expect.objectContaining({ shell: "" }),
    );
    await act(async () => {
      pendingShells.resolve(["/bin/sh"]);
...
function terminalContainer(id: string): ContainerSummary {
  return {
    id,
    name: id,
    image: "test:latest",
    state: "running",
    status: "Up",
    health: "unknown" as ContainerSummary["health"],
