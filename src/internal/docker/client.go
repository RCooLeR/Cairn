...
func (c *Client) connect(ctx context.Context, reuse bool) error {
	c.reconnectMu.Lock()
	defer c.reconnectMu.Unlock()
...
func (c *Client) Close() error {
	// Cancel queued/active lazy setup before waiting on reconnectMu. Otherwise
	// a queued attempt can acquire it after Close and publish a new client
	// behind the runtime's shutdown snapshot.
	c.mu.Lock()
	if c.connecting != nil {
		c.connecting.cancel()
		c.connecting = nil
...
func (c *Client) ensureConnected(ctx context.Context) (APIClient, error) {
	...
	// Inventory, health and metrics can all reach this path at startup. Share
	// the in-flight connection (including failures) instead of queuing a new
	// SDK client and WSL process for every caller behind reconnectMu.
	attempt := c.connecting
	if attempt == nil {
		connectCtx, cancel := context.WithTimeout(context.WithoutCancel(ctx), defaultConnectTimeout)
		attempt = &connectAttempt{done: make(chan struct{}), cancel: cancel}
		c.connecting = attempt
		go c.runConnectAttempt(connectCtx, attempt)
	}
	attempt.waiters++
	c.mu.Unlock()
	defer func() {
...
func (c *Client) runConnectAttempt(ctx context.Context, attempt *connectAttempt) {
...
func newSDKClientWithDialer(host string, dialContext func(context.Context, string, string) (net.Conn, error)) (APIClient, error) {
	...
	if dialContext == nil {
		...
		// Preserve the SDK's platform-specific Unix/named-pipe/TCP dialer,
		// and guard native hijack handshakes as well as WSL/SSH handshakes.
	...
	return dockerclient.New(
		...
		dockerclient.WithScheme("http"),
		dockerclient.WithDialContext(execAttachDialer(dialContext)),
	)
}

func dockerHTTPClient() *http.Client {
	// Docker's hijack protocol is HTTP/1 only. Explicit protocols also prevent
	// Transport.Clone (used by the SDK) from installing a TLS config for HTTP/2.
	// Moby treats any TLS config as a reason to bypass our custom hijack dialer,
	// even when the request scheme is explicitly HTTP.
	...
	return &http.Client{
		Transport: &http.Transport{
			...
			// Keep relays alive across health/background sampling intervals.
			// An idle timeout equal to the 10s ping interval races the next ping
			// and needlessly starts a new WSL/SSH process each cycle.
			IdleConnTimeout: 3 * defaultPingEvery,
		},
