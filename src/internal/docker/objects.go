...
func (c *Client) publishSnapshotChanges(before store.ObjectCacheSnapshot, after store.ObjectCacheSnapshot) {
	for _, change := range []struct {
		...
		// Container cache writes publish their committed deltas directly. A
		// reconciliation-wide comparison would duplicate those notifications.
...
func (c *Client) saveContainers(ctx context.Context, records []store.ContainerCacheRecord, replace bool) error {
	...
	if !replace && len(records) == 0 {
		return nil
	}
	changed, err := cache.SaveContainersWithChangesScoped(ctx, scope, records, c.now(), replace)
	if err != nil {
		return err
	}
	if len(changed) > 0 {
		c.publish(bus.TopicObjectsChanged, ObjectsChangedPayload{Kind: objectKindContainer, IDs: changed})
	}
	return nil
}
