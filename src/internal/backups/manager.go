...
func cleanupExpiredPlan(record planRecord) {
	if err := cleanupDiscardedPlan(record); err != nil {
		slog.Warn("expired backup plan cleanup failed", "planID", record.Plan.PlanID, "error", err)
	}
}
