...
func TestManagerRegistrationPublishesInitializedWriter(t *testing.T) {
	t.Parallel()
	manager := NewManager(fakeProvider{}, nil, nil, nil, Options{})
	t.Cleanup(manager.StopAll)
	reservation, err := manager.reserveSession()
	if err != nil {
		t.Fatal(err)
	}
	pty := newFakePTYSession(PTYSpec{})
	active := &session{
		info:      models.TerminalSessionInfo{ID: "registered"},
		stream:    pty,
		closeDone: make(chan struct{}),
	}
	if err := manager.registerReserved(active, reservation); err != nil {
		t.Fatal(err)
	}
	if active.writeQueue == nil || active.writerDone == nil {
		t.Fatal("published session has an uninitialized writer")
	}
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	if err := manager.WriteTerminal(ctx, active.info.ID, []byte("ready")); err != nil {
		t.Fatalf("write immediately after registration: %v", err)
	}
	if got := pty.written(); got != "ready" {
		t.Fatalf("writer received %q, want ready", got)
	}
	manager.StopAll()
	select {
	case <-active.writerDone:
	default:
		t.Fatal("StopAll returned before the published writer stopped")
	}
}
