...
func (s *windowsPTYSession) Close() error {
	...
	s.closeOnce.Do(func() {
		...
		if s.terminateProcessIfRunning() {
...
func (s *windowsPTYSession) Wait() int {
	s.waitOnce.Do(func() {
		...
		defer s.closeProcessHandle(process)
...
func (s *windowsPTYSession) terminateProcessIfRunning() bool {
	// Wait closes the process handle when the child exits. Keep its handle
	// ownership lock through these nonblocking API calls so Windows cannot
	// recycle the handle for another object before TerminateProcess uses it.
	s.processMu.Lock()
	defer s.processMu.Unlock()
	if s.process == 0 || waitForProcess(s.process, 0) {
		return false
	}
	return windows.TerminateProcess(s.process, 1) == nil
}
