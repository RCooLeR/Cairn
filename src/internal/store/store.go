...
func (s *Store) Migrate(ctx context.Context) error {
	...
	// Existing, current databases were already scrubbed above. Avoid repeating
	// the full-table scans and WAL checkpoint on every ordinary application
	// launch; new or upgraded schemas still receive the post-migration scrub.
	if current == 0 || len(pending) > 0 {
		if err := s.scrubLegacyBuildArgValues(ctx); err != nil {
			return err
		}
	}
...
// migration backup; new and upgraded databases are scrubbed after migrations.
