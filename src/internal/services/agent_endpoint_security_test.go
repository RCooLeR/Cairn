...
func TestCloneAgentHTTPClientEnforcesPolicyWithoutMutatingInjectedClient(t *testing.T) {
	...
	originalTransport := &http.Transport{
		...
		Dial: func(string, string) (net.Conn, error) { //nolint:staticcheck // Deliberately inject legacy hooks to verify they cannot bypass endpoint containment.
			dialCalls.Add(1)
		...
		DialTLS: func(string, string) (net.Conn, error) { //nolint:staticcheck // Legacy TLS hooks must be sanitized along with DialTLSContext.
			dialCalls.Add(1)
