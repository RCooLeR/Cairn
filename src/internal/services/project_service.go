...
func (s *ComposeService) runComposeServiceAction(ctx context.Context, project store.ProjectRecord, action string, command string, run func() (*providers.CommandResult, error)) error {
	...
	auditErr := s.recordComposeServiceAudit(ctx, project, action, command, "success", duration, nil)
	if auditErr != nil {
		s.publishJobDone(jobID, "", auditErr)
	} else {
...
func (s *ProjectService) runProjectAction(ctx context.Context, action string, projectID string, removeVolumes bool, planned *models.CommandPlan, jobID string, expectedGeneration *uint64, expectedFingerprint string) error {
	...
	auditErr := s.recordProjectAudit(operationCtx, project, action, command, plan.Risk, "success", duration, nil)
	if auditErr != nil {
		s.publishProjectJobDone(jobID, project.ID, action, command, "", auditErr)
	} else {
...
func (s *ProjectService) runStaleProjectContainerAction(ctx context.Context, action string, project store.ProjectRecord, removeVolumes bool, planned *models.CommandPlan, jobID string) error {
	...
	auditErr := s.recordProjectAudit(ctx, project, action, command, plan.Risk, "success", duration, nil)
	if auditErr != nil {
		s.publishJobDone(jobID, "", auditErr)
	} else {
		s.publishJobDone(jobID, "success", nil)
