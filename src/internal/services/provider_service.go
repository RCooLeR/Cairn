...
func (s *ProviderService) Detect(ctx context.Context, providerID string) (*models.ProviderStatus, error) {
	if s.Manager != nil {
		previousID := s.Manager.ActiveProviderID(ctx)
		status, err := s.Manager.Detect(ctx, providerID)
		return status, errors.Join(err, s.rebindAfterInitialSelection(ctx, previousID))
	}
	return nil, notReady()
}
...
func (s *ProviderService) DetectAll(ctx context.Context) (map[string]*models.ProviderStatus, error) {
	if s.Manager != nil {
		previousID := s.Manager.ActiveProviderID(ctx)
		statuses, err := s.Manager.DetectAll(ctx)
		return statuses, errors.Join(err, s.rebindAfterInitialSelection(ctx, previousID))
	}
	return nil, notReady()
}

func (s *ProviderService) rebindAfterInitialSelection(ctx context.Context, previousID string) error {
	if s.Manager.ActiveProviderID(ctx) == previousID {
		return nil
	}
...
func (s *ProviderService) runProviderLifecycle(ctx context.Context, action string, providerID string, risk models.Risk) error {
	...
	auditErr := s.recordProviderLifecycleAudit(ctx, action, providerID, command, risk, "success", duration, nil)
	if action == "start" || action == "restart" {
		if s.Manager.ActiveProviderID(ctx) != providerID {
			return auditErr
		}
		return errors.Join(auditErr, s.rebindActiveProvider(ctx))
	}
	if action == "stop" {
		return errors.Join(auditErr, s.clearRuntimeIfActiveProvider(ctx, providerID))
	}
	return auditErr
}
...
func (s *ProviderService) runPlannedProviderLifecycle(ctx context.Context, plan security.ProviderPlan) error {
	...
	auditErr := s.recordProviderLifecycleAudit(ctx, plan.Action, plan.ProviderID, command, plan.Plan.Risk, "success", time.Since(started), nil)
	if plan.Action == "restart" {
		return errors.Join(auditErr, s.rebindActiveProvider(ctx))
	}
	if plan.Action == "stop" {
		return errors.Join(auditErr, s.clearRuntimeIfActiveProvider(ctx, plan.ProviderID))
	}
	return auditErr
}
