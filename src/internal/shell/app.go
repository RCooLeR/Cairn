...
func Run(assets fs.FS) error {
	...
	// Acquire Wails' process-wide single-instance lock before opening SQLite.
	// A second login/manual launcher must not scan or checkpoint the database
	// while the existing process is writing metrics and serving requests.
	...
	// Wails normally invokes OnShutdown, but startup failures can return before
	// that hook runs. Both paths must cancel and join runtime workers before
	// closing their database and event bus.
