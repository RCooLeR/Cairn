...
func TestAppRuntimeLoadsMetricsSettingsForProviderBinding(t *testing.T) {
	...
	if options.VisibleInterval != 7*time.Second || options.BackgroundInterval != 10*time.Second {
		t.Fatalf("metrics sampling intervals = visible %v, background %v; want 7s visible and 10s background", options.VisibleInterval, options.BackgroundInterval)
	}
...
func TestAppRuntimeDefaultBackgroundMetricsAreThrottled(t *testing.T) {
	runtimeController := newAppRuntime(appRuntimeConfig{RootCtx: context.Background()})
	provider := providers.NewWindowsWSL(providers.WindowsWSLOptions{Distro: "Ubuntu"})
	options := runtimeController.metricsManagerOptions(context.Background(), provider, runtimescope.Must(provider.ID(), "wsl:ubuntu"))
	if options.VisibleInterval != 2*time.Second || options.BackgroundInterval != 10*time.Second {
		t.Fatalf("default metrics intervals = visible %v, background %v; want 2s and 10s", options.VisibleInterval, options.BackgroundInterval)
	}
	if !options.DisableStreamingStats || options.StatsConcurrency != 1 {
		t.Fatalf("WSL metrics must retain bounded one-shot sampling: %#v", options)
	}
}
