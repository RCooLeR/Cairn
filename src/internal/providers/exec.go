...
func (ExecRunner) RunWithOptions(ctx context.Context, opts CommandRunOptions, name string, args ...string) (*CommandResult, error) {
	...
	// A WSL/service launcher can leave descendants holding its output pipes
	// open after it exits (or is killed by the context). Bound pipe draining
	// too, otherwise cmd.Run can wait forever past the operation deadline.
