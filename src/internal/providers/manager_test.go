...
func TestManagerDetectionPreservesEstablishedProvider(t *testing.T) {
	for _, selection := range []string{"saved", "explicit", "detected"} {
		t.Run(selection, func(t *testing.T) {
			ctx := context.Background()
			db := openProviderTestStore(t, ctx)
			selected := &fakeProvider{id: "selected", healthy: true}
			other := &fakeProvider{id: "other", healthy: true}
			manager := NewManager(db.Providers(), db.Settings(), []PlatformProvider{other, selected})
			switch selection {
			case "saved":
				if err := db.Settings().SetString(ctx, "provider.active_id", selected.ID()); err != nil {
					t.Fatal(err)
				}
			case "explicit":
				if err := manager.SetActiveProvider(ctx, selected.ID()); err != nil {
					t.Fatal(err)
				}
			case "detected":
				if _, err := manager.Detect(ctx, selected.ID()); err != nil {
					t.Fatal(err)
				}
			}
			selected.healthy = false
			if _, err := manager.Detect(ctx, other.ID()); err != nil {
				t.Fatal(err)
			}
			if got := manager.ActiveProviderID(ctx); got != selected.ID() {
				t.Fatalf("partial detection retargeted runtime to %q", got)
			}
			if _, err := manager.DetectAll(ctx); err != nil {
				t.Fatal(err)
			}
			if got := manager.ActiveProviderID(ctx); got != selected.ID() {
				t.Fatalf("unhealthy selection retargeted runtime to %q", got)
			}
		})
	}
}
...
type incompleteDetectionProvider struct {
	fakeProvider
	returnNil bool
}

func (p *incompleteDetectionProvider) Detect(ctx context.Context) (*models.ProviderStatus, error) {
	if p.returnNil {
		return nil, nil
	}
	<-ctx.Done()
	return &models.ProviderStatus{Problems: []models.ProviderProblem{{Code: ProblemDockerMissing}}}, nil
}

func TestDetectProviderRejectsIncompleteStatusAfterTimeout(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		provider := &incompleteDetectionProvider{fakeProvider: fakeProvider{id: "windows_wsl_ubuntu", kind: TypeWindowsWSL}}
		status, err := detectProvider(context.Background(), provider)
		if status != nil {
			t.Fatalf("detectProvider() retained incomplete status: %#v", status)
		}
		if !errors.Is(err, context.DeadlineExceeded) || !apperror.IsCode(err, apperror.ProviderNotReady) {
			t.Fatalf("detectProvider() error = %v, want retryable deadline error", err)
		}
	})
}

func TestDetectProviderRejectsEmptyStatus(t *testing.T) {
	provider := &incompleteDetectionProvider{fakeProvider: fakeProvider{id: "linux_native"}, returnNil: true}
	status, err := detectProvider(context.Background(), provider)
	if status != nil || !apperror.IsCode(err, apperror.ProviderNotReady) {
		t.Fatalf("detectProvider() = %#v, %v, want no status and provider error", status, err)
	}
}
