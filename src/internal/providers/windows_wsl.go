...
const (
	...
	wslBackendPortDialTimeout    = 10 * time.Second
...
func NewWindowsWSL(opts WindowsWSLOptions) *WindowsWSLProvider {
	...
	return &WindowsWSLProvider{
		...
		ids:           opts.IDs,
		installPlans:  map[string]wslInstallPlan{},
		ipResolveGate: make(chan struct{}, 1),
	}
}
...
func (p *WindowsWSLProvider) SetDistro(distro string) {
	...
	if strings.EqualFold(p.distro, distro) {
		return
	}
	p.distro = distro
...
func (p *WindowsWSLProvider) dialBackendPort(ctx context.Context, network string, port int) (net.Conn, error) {
	ctx, cancel := context.WithTimeout(ctx, wslBackendPortDialTimeout)
	defer cancel()
...
func (p *WindowsWSLProvider) backendIP(ctx context.Context) (string, error) {
	// A browser commonly opens several forwarded connections at once. Only
	// one of them needs to launch WSL to refresh an expired address.
	select {
	case p.ipResolveGate <- struct{}{}:
		defer func() { <-p.ipResolveGate }()
	case <-ctx.Done():
		return "", ctx.Err()
	}
	...
	distro := p.configuredDistro()
	p.ipCacheMu.Lock()
	if p.ipCache.distro == distro && p.ipCache.value != "" && time.Now().Before(p.ipCache.expiresAt) {
	...
	if output, ok := p.runWSLText(ctx, distro, "sh", "-lc", wslDefaultRouteIPCommand); ok {
		if value := firstUsableBackendIPv4(output); value != "" {
			p.cacheBackendIP(distro, value)
			return value, nil
		}
	}
	if err := ctx.Err(); err != nil {
	...
	if output, ok := p.runWSLText(ctx, distro, "sh", "-lc", wslHostnameIPsCommand); ok {
		if value := firstUsableBackendIPv4(output); value != "" {
			p.cacheBackendIP(distro, value)
			return value, nil
		}
...
func (p *WindowsWSLProvider) cacheBackendIP(distro, value string) {
	p.ipCacheMu.Lock()
