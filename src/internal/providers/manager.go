...
type Manager struct {
	...
	detections       map[detectionKey]*providerDetection
}
...
func (m *Manager) Detect(ctx context.Context, providerID string) (*models.ProviderStatus, error) {
	...
	status, err := m.detectShared(ctx, provider)
	if err != nil {
		return nil, err
	}
...
func detectProvider(ctx context.Context, provider PlatformProvider) (*models.ProviderStatus, error) {
	detectCtx, cancel := context.WithTimeout(ctx, detectBudgetFor(provider))
	defer cancel()
	status, err := provider.Detect(detectCtx)
	// Provider probes translate command failures into component problems. An
	// exhausted detection budget is not evidence that those components are
	// missing: do not persist an incomplete probe over the last known status.
	if detectCtx.Err() != nil {
		return nil, apperror.New(apperror.ProviderNotReady, "Provider detection was interrupted",
			apperror.WithCause(detectCtx.Err()),
			apperror.WithRepairHints("Wait for the backend to finish starting, then retry provider detection."))
	}
	if err != nil {
		return nil, err
	}
	if status == nil {
		return nil, apperror.New(apperror.ProviderNotReady, "Provider detection returned no status")
	}
	return status, nil
}
...
func (m *Manager) updateActiveAfterDetect(ctx context.Context, statuses map[string]*models.ProviderStatus) {
	...
	m.mu.Lock()
	defer m.mu.Unlock()
	// Detection refreshes health; it must not silently retarget an established
	// runtime (and its plans, terminals, or container actions) to another engine.
	// Keep valid selections even when stopped or temporarily unreachable.
	if _, ok := m.providers[m.activeID]; ok {
		return
	}
	if _, ok := m.providers[saved]; ok {
		m.activeID = saved
		return
	}
	for _, id := range m.order {
		if status := statuses[id]; status != nil && status.Healthy {
			m.activeID = id
			return
		}
	}
