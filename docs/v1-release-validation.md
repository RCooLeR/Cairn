...
Every push to `main` runs the normal CI matrix on Ubuntu 24.04, Windows, and macOS, then package smoke for NSIS, AppImage, deb, and dmg. The Linux package-smoke leg also runs:

```powershell
./scripts/run-release-validation.ps1 -Suite security,performance,soak-smoke -SoakDuration 30s -SoakTimeout 5m
```
...
Phase 10.3 packaging evidence is green: CI run 27487171104 and marker run 27487366493 passed lint/unit and package smoke on Ubuntu 24.04, Windows, and macOS. Phase 10.4 remains open until the 24 h soak and manual platform rows above are recorded.
