...
Automated local/CI evidence: `npm run test:release-ui` passed on Windows with 15 Playwright checks: 10 route axe scans, command palette/notification/import-modal axe scans, route screenshot stability, route visual regression against committed goldens, a daemon-stopped degraded-mode browser check that verifies every route shows the degraded banner/stale cached-data watermark with no serious axe violations, disables the container mutation, and does not start log/stats streams, plus the seeded browser performance fixture for dashboard, route-switch, inventory-search, and 5,000-line log virtualization budgets. CI run 27499807123 passed the same current 15-check suite through Ubuntu release validation smoke. The previous 14-check route/degraded/visual suite passed inside the `cairn-dev` WSL Docker daemon using `mcr.microsoft.com/playwright:v1.60.0-noble` from a throwaway container-local repo copy before the seeded fixture was added.
