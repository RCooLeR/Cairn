...
| 0 | 0.1 Repo & toolchain | in_progress | `npm audit --audit-level=moderate`; ESLint; Vitest; `tsc --noEmit`; Vite production build; `go test . ./internal/...`; `go vet . ./internal/...`; `go build ./...`; `wails3 build`; `golangci-lint v2.12.2 run --timeout=5m`; CI run 27429536451 green on Ubuntu 24.04, Windows, macOS; `wails3 dev -port 9247` built, launched `bin/cairn.exe`, started Vite, and WebView2 connected | Wails v3 pinned to `v3.0.0-alpha.99`; Go pinned to `toolchain go1.26.4`; logo/icon sourced from `assets/`. CI Linux runner changed to `ubuntu-24.04` in `dev-docs/08-packaging-release.md` because Wails alpha.99 defaults to GTK4/WebKitGTK 6.0. Golangci now excludes frontend dependencies and build outputs. Pending: manual `wails3 dev` hot-reload proof; see `docs/manual-platform-validation.md`. Browser plugin unavailable in this sandbox (`CreateProcessAsUserW failed: 5`). |
