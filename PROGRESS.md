...
| 0 | 0.1 Repo & toolchain | in_progress | `npm audit --audit-level=moderate`; ESLint; Vitest; `tsc --noEmit`; Vite production build; `go test . ./internal/...`; `go vet . ./internal/...`; `go build ./...`; `wails3 build`; `golangci-lint v2.12.2 run --timeout=5m`; CI run 27428985641 found missing Linux GTK/WebKit deps; CI run 27429209269 found golangci scanning `frontend/node_modules` | Wails v3 pinned to `v3.0.0-alpha.99`; Go pinned to `toolchain go1.26.4`; logo/icon sourced from `assets/`. CI Linux runner changed to `ubuntu-24.04` in `dev-docs/08-packaging-release.md` because Wails alpha.99 defaults to GTK4/WebKitGTK 6.0. Golangci now excludes frontend dependencies and build outputs. Pending: `wails3 dev` hot-reload proof and green rerun. Browser plugin unavailable in this sandbox (`CreateProcessAsUserW failed: 5`). |
