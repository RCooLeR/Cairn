...
| 0 | 0.4 Wails services skeleton + bindings | in_progress | `internal/models` DTO/enums from `dev-docs/04-api-contracts.md`; `internal/services` Wails-bound skeleton structs for all 12 API groups; shell registers services and marshals `AppError`; `wails3 generate bindings -clean=true -ts` produced 12 services, 115 methods, 8 enums, 80 models; frontend `api/` wrappers, `useEvent` hook, zustand skeleton stores, and `AppVersion()` render path; `go test . ./internal/...`; `go vet . ./internal/...`; `go build ./...`; `golangci-lint v2.12.2 run --timeout=5m`; frontend ESLint, Vitest, and `tsc --noEmit` | CI binding-diff guard added; pending pushed CI proof before marking green. Local standard Vite production build still has the bundled Codex Node `EPERM` issue documented in 0.3 notes; CI is the canonical normal-build check. |
