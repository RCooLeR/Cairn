...
| 0 | 0.5 Design system base | in_progress | Core UI tokens/components added per `dev-docs/ui/00-design-system.md`; app shell placeholders use logo/icon assets from `assets/`; Ladle component catalog added and wired into CI; frontend ESLint, `tsc --noEmit`, Vitest, `npm audit --audit-level=moderate`; `go test . ./internal/...`; `go vet . ./internal/...`; `go build . ./internal/...`; `golangci-lint run --timeout=5m` with workspace cache | Local Ladle build compiles 1771 modules and reaches Rollup output, then the bundled Codex Node runtime hits `EPERM` writing ignored files under `frontend/dist/ladle`; direct Node/PowerShell writes to the same directory succeed. CI catalog build is pending as the authoritative check. |
