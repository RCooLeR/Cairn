...
| 0 | 0.3 Event bus + error model + logging | green | `internal/bus` topic constants + in-process pub/sub with drop-oldest slow-subscriber policy and coalescing/batching helpers; `internal/apperror` contract error codes with wrap/unwrap helpers; `internal/logging` JSON structured logger with rotating file writer; `go test ./internal/bus ./internal/logging`; `go test` for `internal/apperror` via explicit test binary workaround; `go test` for all other repo packages; `go vet . ./internal/...`; `go build ./...`; `golangci-lint v2.12.2 run --timeout=5m`; frontend ESLint, Vitest, and `tsc --noEmit` | Local standard Vite production build under the bundled Codex Node runtime fails before app output with `EPERM` inside Vite/Rolldown while copying public assets/creating ignored `dist` files; manual Node/PowerShell writes to the same paths succeed. CI normal frontend build remains the canonical check after push. |
